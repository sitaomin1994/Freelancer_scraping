...
each requests
"""
async def get_each_requests(url, h, params, project_id, cursor, connection, index_id):

    #print("before request ", index_id)
    ...
    status, result = await get(url, h, params)
    print(result)
    '''
    async with aiohttp.ClientSession(read_timeout = None, conn_timeout=None) as session:
        async with session.get(url, headers = h, params = params) as response:
            result = await response.json()
            status = response.status
    '''
    ...
    if status == 200:
        ...
        for index, bid in enumerate(bids):
            ...
            if (bid['bidder_id'] != None):
                ...
                if user_id in users:
                    ...
                    result.extend(
                        ["null", "null", "null", -1, "null", "null", -1, "null", "null", "null", False, "null", "null",
                         "null", "null"])
            else:
                result.extend(
                    ["null", "null", "null", -1, "null", "null", -1, "null", "null", "null", False, "null", "null",
            ...
            SQL_insert_bids = "REPLACE INTO BIDS(bids_id, project_id, user_id, position_rank, time_submitted, amount_offered," \
                              ...
                              " %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s," \
                              " %s, %s, '%s', '%s', '%s',  %s, '%s', '%s', %s, '%s', '%s', '%s', %s, '%s','%s','%s','%s')" % tuple(
                result)
        ...
        print("request error project",index_id)
        #time.sleep(5)
        ERROR_SIGN_REQUEST = True

    # save information to BIDS_PROGRESS
    if (ERROR_SIGN_BIDS == False and ERROR_SIGN_REQUEST == False):
        SQL_SAVE_PROGRESS = "UPDATE BIDS_PROGRESS SET extracted = 1, bids_count = %s, error_type = 'null', error_details = 'null' WHERE project_id = %s;" % (
        bids_count, project_id)
    elif ERROR_SIGN_REQUEST == True:
        SQL_SAVE_PROGRESS = "UPDATE BIDS_PROGRESS SET extracted = 0, error_type = 'request_error' WHERE project_id = %s;" % (
            project_id)
    else:
        SQL_SAVE_PROGRESS = "UPDATE BIDS_PROGRESS SET extracted = 0, error_type = 'bids_error', error_details = '%s', bids_count = %S WHERE project_id = %s;" % (
        json.dumps(error_set), bids_count, project_id, json.dumps(error_set))

    # print(SQL_SAVE_PROGRESS)
    print(ERROR_SIGN_BIDS)
    print(ERROR_SIGN_REQUEST)
    print("project %d finished" % (index_id))
...
def get_bids_info(project_list, times, cursor, connection):
    ...
    for i in range(times):
        ...
        urls.append('https://www.freelancer.com/api/projects/0.1/projects/' + str(project_id) + '/bids/')
...
main program
'''
def main_programm(first_time_sign, update_progress, number_to_extracted):
...
times = int(input("how many times do you want to run main program?"))
print("###########################################################################################################################")
