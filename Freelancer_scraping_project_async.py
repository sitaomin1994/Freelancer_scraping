...
each requests
"""
async def get_each_requests(url, h, params , cursor, connection, index_id, projects_id):
    ...
    if status == 200:
        projects = result_json["result"]["projects"]
        selected_bids = result_json["result"]["selected_bids"]
        users = result_json['result']['users']

        number_of_project = len(projects)
        ...
        for iter, project in enumerate(projects):
            ...
            try:
                result.append(project['currency']['code'] if project['currency'][
                                                                 'code'] != None else 'null')  # currency_code - string
            except:
                result.append("null")
            ...
            try:
                result.append(project['upgrades']['urgent'] if project['upgrades'][
                                                                   'urgent'] != None else None)  # urgent - boolean
            except:
                result.append(None)
            ...
            result.append(
                project['type'] if project['type'] != None else 'null')  # project_type - string - fixed or hourly
            result.append(project['hireme'] if project['hireme'] != None else None)  # hireme - boolean
            result.append(
                json.dumps(project['recommended_freelancers']).replace("'", ""))  # recommended_freelancer --- string

            try:
                result.append(json.dumps([ele['category']['name'] for ele in project['jobs']]).replace("'",
            ...
            result.append(convert_utc_to_date(project['time_free_bids_expire'] if project[
                'time_free_bids_expire'] else 0))  # time free bids expire --- number
            result.append(project['sub_status'] if project[
                                                       'sub_status'] != None else 'null')  # sub status --- string --- eg. closed_awarded
            result.append(
                project['language'].replace("'", "") if project['language'] != None else 'null')  # language --- string
            result.append(
                project['seo_url'].replace("'", "") if project['seo_url'] != None else 'null')  # project_url --- string
            result.append(convert_utc_to_date(project['time_submitted']
                                              if project[
                                                     'time_submitted'] != None else 0))  # project_time_submitted - number

            try:
                result.append(project['budget']['minimum'] if project['budget'][
                                                                  'minimum'] != None else -1)  # budget_minimum --- float
            except:
                result.append(-1)

            try:
                result.append(project['budget']['maximum'] if project['budget'][
                                                                  'maximum'] != None else -1)  # budget_maximum --- float
            except:
                result.append(-1)
            ...
            try:
                result.append(project['bid_stats']['bid_avg'] if project['bid_stats'][
                                                                     'bid_avg'] != None else -1)  # bids_status_avg ---- float
            except:
                result.append(-1)
            ...
            try:
                result.append(convert_utc_to_date(project['hireme_initial_bid']['time_awarded']
                                                  if project['hireme_initial_bid'][
                                                         'time_awarded'] is not None else 0))  # hireme_initail time awarded
            except:
            ...
            sql = "REPLACE INTO PROJECT(project_id, owner_id, bid_period, title, currency_code, currency_rate," \
                  ...
                  "VALUES (%s, %s, %s, '%s', '%s', %s, '%s', %s, '%s', '%s', '%s', %s, '%s', '%s', '%s', '%s', '%s', '%s', '%s', '%s', '%s', '%s', %s, %s, %s, %s, '%s', %s, '%s', %s)" % tuple(
                result)
        ...
        if selected_bids != None:
            for key, values in selected_bids.items():
                ...
                try:
                    ...
                    selected_bid_id = -1
                try:
                    selected_bid_time_awarded = values[0]["time_awarded"] if values[0][
                                                                                 "time_awarded"] is not None else 0
                except:
                ...
                sql = "UPDATE PROJECT SET selected_bids_time_awarded = '%s', selected_bids_id = %s, selected_bids_bidder_id = %s,number_of_selected_bids = %s" \
                      " WHERE project_id = %s" % (
                      convert_utc_to_date(selected_bid_time_awarded), selected_bid_id, selectd_bid_bidder_id,
                      number_of_bids, key)
                # print(sql)
        ...
        if users != None:
            ...
            for key, values in users.items():
                ...
                try:
                    user_info.append(values['employer_reputation']['entire_history']['reviews'] if
                                     values['employer_reputation']['entire_history']['reviews'] is not None else -1)
                except:
                    user_info.append(-1)

                try:
                    user_info.append(values['employer_reputation']['entire_history']['overall'] if
                                     values['employer_reputation']['entire_history']['overall'] is not None else -1)
                except:
                    user_info.append(-1)
                ...
                user_info.append(
                    convert_utc_to_date(values['registration_date'] if values['registration_date'] is not None else 0))
                ...
                SQL_insert_user_info = "UPDATE PROJECT SET employer_verifications = '%s', employer_username = '%s', employer_displayname = '%s', " \
                                       ...
                                       "employer_country = '%s',  employer_since = '%s' WHERE owner_id = %s" % (
                                           tuple(user_info))
        ...
        print("request error happen on round", index_id)
    ...
    print("number of project extracted:", number_of_project)
    print("====================================================================================================================================")


'''
function to get projects info
'''
def get_project_info(project_list, cursor, connection):
    ...
    for i in range(times+1):
        #url
        urls.append('https://www.freelancer.com/api/projects/0.1/projects/')
        #header
        token_index, token = generate_new_token(token_index)
        h = {"Freelancer-OAuth-V1": token}
        header_list.append(h)
        #params
        projects = project_list[i * 100:(i + 1) * 100]
        projects_pass = [('projects[]', str(project_id)) for project_id in projects]
        print(projects_pass)
        params = [('frontend_project_statuses[]', '2'),
        ...
        params.extend(projects_pass)
        params_list.append(params)

        #project_list
        project_id_list.append(projects)
    ...
    tasks = [asyncio.ensure_future(get_each_requests(url, header, params, cursor, connection, index, projects_id)) for url, header, params, index, projects_id in zip(urls, header_list, params_list, index_list, project_id_list)]
...
get_project_info(project_list, cursor, connection)
